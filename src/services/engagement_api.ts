  ...
  static async getEngagements(page: number, setData: any, setTempData: any) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        let engagements: Engagement[] = responseJson.map(
  ...
  static async getDetailEngagement(engagementId: string, setData: any) {
    ...
    try {
      ...
      if (response.status === 200) {
        let responseJson = response.data.data;
        // change to model engagement
        let engagements: DataEngagement[] = responseJson.map(
          (item: any) => item as DataEngagement
        );

        setData(BaseState.success(engagements, 1, 1, response.data.totalItems));
      } else {
