...
import {CardEngagement, CardEngagementLoading} from "../components/card_engagement";
...
function MainScreen() {
    ...
    useEffect(() => {
        EngagementApi.getEngagements(1, setData,setTempData)
    }, [],)
                ...
                <div className=" flex flex-wrap items-center justify-center md:justify-between mx-auto p-5">
                    <div className={"flex"}>
                        ...
                        <a href="/" className="flex items-center space-x-3 rtl:space-x-reverse">
                            <span className="self-center text-2xl font-semibold whitespace-nowrap">{ConstantApp.NAME}</span>
                        </a>
                    </div>

                    <div className={"flex items-center w-full md:w-[30%] p-2"}>
                        <FontAwesomeIcon icon={faSearch} className={"hover:cursor-pointer"}/>
                        <input className={"bg-gray-100 rounded-lg border-2 shadow-sm border-gray-200 w-full p-2 mx-2"}
                               type="text" name="search" id="search" placeholder={"Silahkan cari dengan username"}
